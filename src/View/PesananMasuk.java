...
import Controller.PesananMasukController;
 ...
 * @author glenn
 */
public class PesananMasuk extends JFrame implements MouseListener{
    JFrame frame;
    JTable table;
    JLabel exit;
    PesananMasukController masuk = new PesananMasukController();
    public static void main(String[] args) {
        new PesananMasuk();
    }
    
    PesananMasuk() {
        frame = new JFrame("Pesanan Masuk");
        frame.setSize(1100, 490);
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        frame.setLocationRelativeTo(null);
        
        String[] column = {"ID Pesanan", "Nama", "Produk", "Ukuran", "Jumlah", "Harga Total"};
        ArrayList<Pesanan> pesananList = masuk.getPesananMasukList();
        ...
        for (int i = rData.length-1; i >= 0; i--) {
            ...
            Customer customer = masuk.getCustomer(cutomerId);
            rData[i][1] = customer.getName();
            int produkId = pesanan.getBarangId();
            Produk produk = masuk.getProduk(produkId);
